AC_DEFUN([AC_DEFINE_DIR],
	...
	while test "x${last_ac_define_dir}" != "x${ac_define_dir}"; do
		...
		AS_VAR_IF([ac_define_dir_counter],
			[128],
...
AC_DEFUN([RB_PROTO_INET6],
	[AC_CACHE_CHECK([for INET6 protocol support], [rb_cv_proto_inet6], [AC_TRY_CPP([
#ifndef _WIN32
#include <sys/types.h>
...
AC_DEFUN([RB_CHECK_TIMER_CREATE],
	[AC_CACHE_CHECK([for a working timer_create(CLOCK_REALTIME)], [rb__cv_timer_create_works], [AC_TRY_RUN([
#ifdef HAVE_TIME_H
#include <time.h>
#endif
...
AC_DEFUN([RB_CHECK_TIMERFD_CREATE],
	[AC_CACHE_CHECK([for a working timerfd_create(CLOCK_REALTIME)],	[rb__cv_timerfd_create_works], [AC_TRY_RUN([
#ifdef HAVE_TIME_H
#include <time.h>
#endif
...
int main(int argc, char *argv[])
{
	...
	yes)
		AC_DEFINE([USE_TIMERFD_CREATE], [1], [Define to 1 if we can use timerfd_create(CLOCK_REALTIME,...)])
		;;
	esac])
